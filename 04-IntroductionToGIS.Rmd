...
### Something very important about coordinates and projections

Any spatial data collected using GPS devices or downloaded online must have a projection system that links the coordinates to specific locations on the earth's surface. Without a coordinate reference system, we cannot locate places on Earth. 
...
tm_shape(habitat) +
  tm_polygons(fill = "HABITAT", fill.scale = tm_scale(values = "brewer.greens"), fill.legend = tm_legend(title ="Habitat type")) +
tm_shape(roads) +
  tm_lines(col = "ROAD_USE", col.scale = tm_scale(values = "brewer.set1"), col.legend = tm_legend(title = "Road use")) +
tm_shape(fences) +
  tm_lines(lty = 'solid', col = "black") +
tm_shape(birds) +
  tm_dots(fill = 'red', size = 0.5) +
tm_compass(position = c("right", "top")) +
tm_scalebar(position = c("right", "bottom"), width = 21, breaks = c(0,0.5,1, 1.5,2)) + tm_layout(frame = FALSE, legend.outside = T, legend.frame = F, legend.title.size = 1, legend.text.size = 0.7) + tm_add_legend(type = c("symbols"), fill = c('red'), labels = c("Bird locations")) + tm_add_legend(type = c("lines"), fill = c('red'), labels = c("Fences"))
```
...
tm_shape(fences) +
  tm_lines(lty = 'solid', col = "black") +
tm_shape(birds) +
  tm_markers() + tm_add_legend(type = c("symbols"), fill = c('blue'), labels = c("Bird locations")) + tm_add_legend(type = c("lines"), fill = c('black'), labels = c("Fences"))
```
...
```{r}
tmap_mode(mode="plot")
tm_shape(prop) + tm_polygons(fill = "LU", col = "white", fill.scale = tm_scale(values = "brewer.greys"), fill.legend = tm_legend(title ="Land use")) +
tm_scalebar(position = c("left", "bottom"), width = 200, breaks = c(0,10,20,30,40)) +
tm_compass (position = c("left","top")) +
tm_shape(SR) + tm_bubbles(col="white", size = "2016", size.scale = tm_scale_intervals(breaks = c(0,0.990001,3.90001,5.90001,7.90001,9.90001,13), values.range = c(0,0.8)), size.legend = tm_legend(show = F), fill = "2016", fill.scale = tm_scale(values = RdYlGn(6), breaks = c(0,0.990001,3.90001,5.90001,7.90001,9.90001,13)), fill.legend = tm_legend(title = "Sp richness", show = F)) +
  tm_add_legend(type = "symbols", title = "Sp. richness",  fill = RdYlGn(6), size = c(0.4,0.5,0.6,0.7,0.8,0.9), labels = c("0", "1-4","4-6","6-8","8-10","10-13"), col="white") +
    tm_layout(frame = F, legend.outside = T, legend.frame = F, legend.title.size = 1, legend.text.size = 0.7) + tm_title(2016)
```
...
```{r}
map <- tm_shape(prop) + tm_polygons(fill = "LU", col = "white", fill.scale = tm_scale(values = "brewer.greys"), fill.legend = tm_legend(title ="Land use")) +
  tm_scalebar(position = c("left", "bottom"), width = 200, breaks = c(0,10,20,30,40)) +
  tm_compass (position = c("left","top")) +
  tm_shape(SR2) + tm_bubbles(col="white", size = "SR", size.scale = tm_scale_intervals(breaks = c(0,0.990001,3.90001,5.90001,7.90001,9.90001,13), values.range = c(0,0.8)), size.legend = tm_legend(show = F), fill = "SR", fill.scale = tm_scale(values = RdYlGn(6), breaks = c(0,0.990001,3.90001,5.90001,7.90001,9.90001,13)), fill.legend = tm_legend(title = "Sp richness", show = F)) +
  tm_add_legend(type = "symbols", title = "Sp. richness",  fill = RdYlGn(6), size = c(0.4,0.5,0.6,0.7,0.8,0.9), labels = c("0", "1-4","4-6","6-8","8-10","10-13"), col="white") +
  tm_layout(frame = F, legend.outside = T, legend.frame = F,legend.title.size = 1, legend.text.size = 0.7)  +
  tm_facets_grid(pages = "Year", free.coords = FALSE)
